...
supersig <- rownames(res)[which(-log10(res$pvalue)>=5.67)]
supersig
...
ggplot(data = as.data.frame(res), aes(x = log2FoldChange, y = -log10(pvalue), color = siggene)) + 
  ...
  geom_hline(yintercept = 5.67, linetype = "dashed") +
...
supersig <- rownames(res)[which(-log10(res$pvalue)>=3.53)]
supersig
...
ggplot(data = as.data.frame(res), aes(x = log2FoldChange, y = -log10(pvalue), color = siggene)) + 
  ...
  geom_vline(xintercept = -1, color = "blue3", linetype = "dashed") +
  scale_color_manual(values = c("blue3", "gray54", "red3")) +
  geom_hline(yintercept = 3.53, linetype = "dashed") +
  geom_text_repel(aes(label = ifelse(name%in%supersig, name, "")))
dev.off()
...
supersig <- rownames(res)[which(-log10(res$pvalue)>=6.09)]
supersig
...
ggplot(data = as.data.frame(res), aes(x = log2FoldChange, y = -log10(pvalue), color = siggene)) + 
  ...
  geom_vline(xintercept = -1, color = "blue3", linetype = "dashed") +
  scale_color_manual(values = c("blue3", "gray54", "red3")) +
  geom_hline(yintercept = 6.09, linetype = "dashed") +
  geom_text_repel(aes(label = ifelse(name%in%supersig, name, "")))
dev.off()
